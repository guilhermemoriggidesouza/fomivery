...
export default class Menu {
  ...
  getRandomSugestion() {
    ...
    const sugestion =
      this.sugestionCases[
        Math.floor(Math.random() * this.sugestionCases.length)
      ];
    const listIds = sugestion!.sequencyIds.split(",");
    this.totalSugested = sugestion!.total;
    this.productsSugestion = this.products.filter((product) =>
      listIds.includes(product.id.toString()),
    );
  }
