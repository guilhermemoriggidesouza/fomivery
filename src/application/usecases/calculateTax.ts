...
export default class CalculateTax {
    ...
    constructor(public readonly googleMaps: Client, private readonly orgRepository: OrgRepository) {

    }

    async execute(input: inputDTO): Promise<outputDTO> {
        try {
            const { data: dataClientAddress } = await this.googleMaps.findPlaceFromText({
                params: {
            ...
            if (dataClientAddress.candidates.length == 0 || dataOrgAddress.candidates.length == 0) {
